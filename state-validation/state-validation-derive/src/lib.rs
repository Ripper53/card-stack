    ...
    let state_conversions = match &ast.data {
        syn::Data::Struct(s) => {
            ...
            for (i, m) in FieldCombinationIter::new(iter).enumerate() {
                let mut create_structs = |combined_struct_name: &Ident,
                                          remainder_struct_name: &Ident,
                                          Meow {
                                              current_field:
                                                  (field_name, field_type, field_generics),
                                              other_fields:
                                                  (
                    other_field_names,
                    other_field_types,
                ...
                match m {
                    M::Next { field_number, meow } => {
                        ...
                        create_structs(&combined_struct_name, &remainder_struct_name, &meow);
                    }
                    M::NewType {
                        field_number,
                        type_number,
