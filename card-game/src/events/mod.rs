...
impl<State: 'static, Ev: Event<State>, Output: 'static> EventManager<State, Ev, Output> {
    ...
    where
        ...
        DynEventListener<State, EvA, OutputA>: NewOutputTrait<State, EvA, Ev, Output>,
        DynEventListener<State, EvB, OutputB>: NewOutputTrait<State, EvB, Ev, Output>,
    {
    ...
    pub fn combine<NewEv: Event<State>, NewOutput: Into<Output> + 'static>(
        mut self,
        new_event_manager: &EventManager<State, NewEv, NewOutput>,
    ) -> Self
    where
        Ev: Into<NewEv>,
        Ev::Input: Into<NewEv::Input>,
        DynEventListener<State, NewEv, NewOutput>: NewOutputTrait<State, NewEv, Ev, Output>,
    {
...
trait NewOutputTrait<State, Ev: Event<State>, NewEv: Event<State> + Into<Ev>, NewOutput>
where
    NewEv::Input: Into<Ev::Input>,
{
    fn new_output(self) -> DynEventListener<State, NewEv, NewOutput>;
}
impl<
    State: 'static,
    Ev: Event<State>,
    ...
    Output: 'static,
    NewOutput: 'static,
> NewOutputTrait<State, Ev, NewEv, NewOutput> for DynEventListener<State, Ev, Output>
where
    NewEv::Input: Into<Ev::Input>,
